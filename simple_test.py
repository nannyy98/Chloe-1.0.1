#!/usr/bin/env python3
"""
Simple System Test - Базовое тестирование системы Chloe AI
"""
...
async def simple_system_test():
    ...
    print("🧪 Chloe AI - Simple System Test")
    print("=" * 50)
    ...
    try:
        ...
        # Test decision engine framework
        try:
        ...
        tools_to_test = [
            ...
            ("Data Analysis Agent", "agents.data_agent.DataAnalysisAgent")
        ]
        ...
        try:
            ...
            if "interfaces" not in test_results:
                test_results["interfaces"] = {}
        ...
        component_scores = [
            list(test_results["core_components"].values()),
            list(test_results["tools"].values()) if isinstance(test_results["tools"], dict) else [],
            list(test_results["memory"].values()) if isinstance(test_results["memory"], dict) else [],
            list(test_results["learning"].values()) if isinstance(test_results["learning"], dict) else []
        ]
        ...
        print(f"🔧 Working Tools: {test_results['tools'].get('successful', 0)}/{test_results['tools'].get('total', 0)}")
        print(f"💾 Memory System: {test_results['memory'].get('status', 'unknown')}")
        print(f"🧠 Learning Engine: {test_results['learning'].get('status', 'unknown')}")
        ...
        if test_results["memory"].get("status") != "success":
            print("   • Check data directory permissions")
            print("   • Ensure SQLite is working properly")
        
        print(f"\n🚀 NEXT STEPS:")
        print("   1. Add your OpenAI API key to .env file")
        print("   2. Run 'python3 cli.py' for interactive mode")
        print("   3. Run 'python3 test_system.py' for detailed testing")
...
if __name__ == "__main__":
    print("Starting Chloe AI Simple Test...")
    try:
        # Проверяем, запущен ли уже event loop
        try:
            loop = asyncio.get_running_loop()
            # Если уже есть loop, используем его
            results = loop.run_until_complete(simple_system_test())
        except RuntimeError:
            # Если нет активного loop, создаем новый
            results = asyncio.run(simple_system_test())
    except Exception as e:
